...
from .bbo_elements import (
    BBOProcessStartEvent,
    BBOFlowElement,
    BBOSubProcessStartEvent,
    BBONormalSequenceFlow,
    BBOConditionalSequenceFlow,
    BBOEndEvent,
    BBOScriptTask,
    BBOSubProcess,
    BBOUserTask,
...
class ProcessEngine:
    ...
    def __init__(self, uimodel: AppInternalStaticModel):
        ...
        self._output_knowledge_graph: Graph = Graph()
    ...
    def output_graph_store(self) -> Graph:
        return self._output_knowledge_graph

    @output_graph_store.setter
    def output_graph_store(self, value: Graph):
        self._output_knowledge_graph = value
    ...
    def process_received_client_data(self, frontend_message: AppExchangeFrontEndData):
        ...
        """
        logger.debug(
            f"Message from the frontend has the type:\n {frontend_message.message_type}"
        )
        logger.debug(
            f"Message from the frontend has the content:\n {frontend_message.message_content}"
        )
        ...
        if frontend_message.message_type == "initiate_exchange":
            ...
            action_node: URIRef = URIRef(
                frontend_message.message_content["action_graph_node"]
            )
            action: OBOPAction = next(
                (
                    f
                    for f in self.internal_app_static_model.actions
                    if f.graph_node == action_node
                ),
                None,
            )
            if action is None:
            ...
            if (
                ...
                and self.app_state.is_waiting_for_form_data
            ):
    ...
    def move_token(self):
        ...
        """
        logger.debug(
            f"Move_token with the current flow element \
        ...
        if self.app_state.current_flow_element is None:
            logger.error(
                "The current flow element is None. The application is not running."
            )
    ...
    def update_current_flow_element(self):
        ...
        next_flow: BBOFlowElement = next(
            (
                f
                for f in self.app_state.current_flow_element_container.flow_elements
                if (
                    isinstance(f, BBONormalSequenceFlow)
                    and f.source_ref.graph_node
                    == self.app_state.current_flow_element.graph_node
                )
                or
                ...
                (
                    isinstance(f, BBOConditionalSequenceFlow)
                    and str(self.app_state.received_action.graph_node)
                    == f.conditional_expression.expression
                    and f.source_ref.graph_node
                    == self.app_state.current_flow_element.graph_node
                )
            ),
    ...
    def execute_script_task(self, script_task: BBOScriptTask):
        ...
        """
        logger.debug(
            f"Executing script task {self.app_state.current_flow_element.graph_node}"
        )
        ...
        if script_task.obop_action is None:
            ...
            logger.debug(
                f"The script {self.app_state.current_flow_element.graph_node} has no action"
            )
            pass
        elif script_task.obop_action.type == "generate_json_form":
            self.execute_generate_json_form_action(script_task)
        elif (
            script_task.obop_action.type == "submit"
            and self.app_state.is_waiting_to_process_front_end_data
        ):
    ...
    def execute_generate_json_form_action(self, script_task: BBOScriptTask):
        logger.debug(
            f"Double-check the script task {str(script_task.graph_node)} and it's actions in the knowledge graph"
        )
        ...
        try:
            result_generator = (
                self.internal_app_static_model.rdf_pellet_reasoning_world.sparql(
                    related_obop_action
                )
            )
            ...
            for row in result_generator:
                ...
                if action.iri == str(script_task.obop_action.graph_node):
                    form_object = next(
                        (
                            c
                            for c in self.internal_app_static_model.forms
                            if str(c.graph_node) == block.iri
                        ),
                        None,
                    )
                else:
            ...
            if form_object is not None:
                json_form: FunctionalJSONForm = (
                    form_object.create_functional_json_form_schemas(self.app_state)
                )
                ...
                self.app_state.layout_type = "form"
                self.app_state.is_waiting_to_send_data = True


        except Exception as e:
            logger.error(
                f"Problem with the relation of OBOP actions with BBO tasks: {e}"
            )
    ...
    def execute_submit_action(self):
        ...
        if (
            action_message["action_type"] == "submit"
            and self.app_state.is_waiting_to_process_front_end_data
        ):
            ...
            form: Form = next(
                (
                    ...
                    if str(form.graph_node) == form_graph_node
                ),
                None,
            )
            if form is not None:
                ...
                if active_form is None:
                    logger.error(
                        f"Active form with graph node {form_graph_node} not found in the application state."
                    )
                if not active_form.has_stored_instances:
                    ...
                    logger.debug(
                        f"New instance created for target classes has uri {named_individual_iri}"
                    )
                    for target_class in form.target_classes:
                        self.output_graph_store.add(
                            (named_individual_iri, RDF.type, target_class)
                        )
                    active_form.has_stored_instances = True
                    active_form.stored_instance_graph_node = named_individual_iri
                    logger.debug(
                        f"Active form {form_graph_node} has stored instances now."
                    )
                else:
                ...
                for key in form_data.keys():
                    ...
                    element_graph_node_uri = next(
                        (
                            k
                            for k, v in self.app_state.current_json_form_name_mapping.items()
                            if v == key
                        ),
                        None,
                    )
                    logger.debug(
                        f"Graph node URI of the element for the key {key} is {element_graph_node_uri}"
                    )
                    for (
                        data_property
                    ) in self.internal_app_static_model.rdf_graph_rdflib.objects(
                        URIRef(element_graph_node_uri), OBOP.containsDatatype
                    ):
                        logger.debug(
                            f"Data property found for the element {element_graph_node_uri}: {data_property}"
                        )
                        # Add the data to the output graph store
                        self.output_graph_store.add(
                            (
                                named_individual_iri,
                                data_property,
                                Literal(form_data[key]),
                            )
                        )
                        logger.debug(
                            f"Added data {form_data[key]} for element {element_graph_node_uri} to the output store."
                        )
    ...
    def validate_output_knowledge_graph(self):
        """_summary_
        Validates the output knowledge graph against the shacl
        shapes defined in the application static model.
        """
        logger.debug("Validating the output knowledge graph using SHACL shapes")
        data_graph = self.output_graph_store
        conforms, results_graph, results_text = validate(
            data_graph,
            shacl_graph=self.internal_app_static_model.shacl_shapes_rdflib,
            ont_graph=None,  # optional, for OWL imports
            inference="rdfs",  # 'rdfs', 'owlrl', 'none'
            abort_on_first=False,  # stop after first error
            meta_shacl=False,  # validate the SHACL itself
            advanced=True,  # allow SHACL advanced features
            js=False,  # disable SHACL-JS (default)
        )        
        json_form : FunctionalJSONForm = {
            "graph_node": "",
            "schema": {
        ...
        self.app_state.json_form = json_form
        self.app_state.layout_type = "message-box"
        self.app_state.is_waiting_to_send_data = True
    ...
    def generate_layout(self) -> AppExchangeGetOutput:
        ...
        if (
            isinstance(self.app_state.current_flow_element, BBOUserTask)
            and self.app_state.is_waiting_to_send_data
        ):
            # The backend is waiting to send data to the frontend
            if self.app_state.layout_type == "message-box":
            ...
            elif self.app_state.layout_type == "form":
                output_message = AppExchangeGetOutput(
                    message_type="layout",
                    layout_type="form",
                    message_content=self.app_state.json_form,
            ...
            logger.debug(
                f" Created response including the form is {jsonpickle.encode(output_message, indent=2)}"
            )
